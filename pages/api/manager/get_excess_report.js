    ...
    try {
        ...
        excessQuery = `
            SELECT
                ii.ingredient_id,
                ii.ingredient_name,
                SUM(CASE WHEN o.order_date >= ? THEN oi.num_items ELSE 0 END) AS total_items_sold,
            ...
            HAVING
                SUM(CASE WHEN o.order_date >= ? THEN oi.num_items ELSE 0 END) <
                0.1 * (SUM(CASE WHEN o.order_date >= ? THEN oi.num_items ELSE 0 END) + ii.ingredient_count)
            ORDER BY
                total_items_sold;
        `;

        reportParameters = [req.start_date, req.start_date, req.start_date, req.start_date];
